...
static void print_usage(int, char **argv) {
  ...
  printf(
      ...
      "--output-format:       enable format output. More detail, see "
      "description\n"
      "--output-format-type:  .i.e: harmony. More detail, see description\n"
      "\n"  // context
...
std::string harmony_decode(std::string &text) {
  // https://cookbook.openai.com/articles/openai-harmony
  std::regex func_regex(R"raw(to=functions\.([a-zA-Z0-9_]+))raw");
  std::regex json_regex(R"raw(<\|message\|>(\{.*\}))raw");

  std::regex final_regex(R"raw(<\|channel\|>final<\|message\|>(.*))raw");
  std::regex final_regex_structure(
      R"raw(<\|channel\|>final <\|constrain\|>(.*)<\|message\|>(.*))raw");

  std::smatch func_match, json_match;

  if (std::regex_search(text, func_match, func_regex) &&
      std::regex_search(text, json_match, json_regex)) {
    std::string func_name = func_match[1];
    std::string json_body = json_match[1];

    json j;
    j["func"] = func_name;
    // Parse the JSON body
    try {
      j["params"] = json::parse(json_body);
    ...
    return j.dump(2);
                 ...
                 text, func_match,
                 final_regex)) {  // <|channel|>final<|message|>✅ “light 01”
                                  // has been turned on.
    std::string final_message = func_match[1];
    return final_message;
  } else if (std::regex_search(text, func_match, final_regex_structure)) {
    std::string final_message = func_match[2];
    // std::cout << "Final message structure: " << final_message << std::endl;
    json j;
    j = json::parse(final_message);
    return j.dump(2);
  } else {
    std::cerr << "No valid function or final message found in the text."
              << std::endl;
    return "";
  }
}
...
int main(int argc, char **argv) {
  ...
  // Optional: Turn off buffering for proper UTF-8 output
  ...
  try {
    ...
    if (cparams.interactive) {
      ...
      auto gen_text_until_eog =
          [&model, &ctx, &smpl](
              llama_batch &batch,
              std::function<int(int, const std::string &)> func_ = nullptr) {
            ...
            do {
              ...
              char buf[100];
              int n = llama_token_to_piece(vocab, new_token, buf, sizeof(buf),
                                           0, true);
              if (n < 0) {
              ...
              if (func_) {
                if (func_(0, out) < 0) {
                  fprintf(stderr, "%s, terminated by caller \n", __func__);
                  exit(0);
                }
              }
      ...
      while (true) {
        ...
        std::string user_prefix = "[developer]";

        std::cout << "\n[avllm_chat] >";
        std::getline(std::cin, line);
        if (line.empty()) continue;

        // trim user_prefix of line if any
        if (line.find(user_prefix) == 0) {
          line = line.substr(user_prefix.size());
          is_developer = true;
          av_trace << "line: " << line << std::endl;
        }
        ...
        if (line[0] == '@') {
          ...
          json j_a = json::array();
          j_a.push_back({{"role", is_developer ? "developer" : "user"},
                         {"content", line}});
          oai_input_js_ = {{"messages", j_a}};
        }

        av_trace << oai_input_js_.dump(2) << std::endl;
        ...
        std::string gen_text;
        std::function<int(int, const std::string &)> gen_text_cb =
            [&gen_text](int, const std::string &out) -> int {
          gen_text += out;
          return 0;
        };

        gen_text_until_eog(batch_, gen_text_cb);

        if (cparams.output_format) {
          if (cparams.output_format_type == "harmony") {
            std::cout << "\n"
                      << COLOR_RED << harmony_decode(gen_text) << COLOR_RESET
                      << std::endl;
          }
        }
      }
