...
int main(int argc, char ** argv)
{
    ...
    try
    {
        ...
        if (cparams.interactive)
        {
            ...
            std::vector<common_chat_msg> common_chat_messages = common_chat_msgs_parse_oaicompat(oai_js.at("messages"));

            auto oai_js_to_model_text = [&model, &template_jinja, &tmpls](const json & oai_js) {
                std::vector<common_chat_msg> messages = common_chat_msgs_parse_oaicompat(oai_js.at("messages"));
                std::vector<common_chat_tool> tools   = oai_js.contains("tools")
            ...
            auto tokenize = [&ctx, &model](const std::string & text) {
                ...
                int n_prompt_tokens       = -llama_tokenize(vocab, &text[0], text.size(), NULL, 0, is_first, true);
                std::vector<llama_token> prompt_tokens(n_prompt_tokens);
                if (llama_tokenize(vocab, &text[0], text.size(), prompt_tokens.data(), prompt_tokens.size(), is_first, true) < 0)
                {
            ...
            while (true)
            {
                ...
                std::string msg_str;
                std::vector<common_chat_msg> common_chat_msg__ = common_chat_msgs_parse_oaicompat(oai_input_js_.at("messages"));

                for (auto it = common_chat_msg__.begin(); it != common_chat_msg__.end(); it++)
                {
                    bool add_ass = std::next(it) == common_chat_msg__.end();
                    msg_str += common_chat_format_single(tmpls.get(), common_chat_messages, *it, add_ass, true);
                }
                ...
                llama_batch batch_ = llama_batch_get_one(prompt_tokens.data(), prompt_tokens.size());
        ...
        try
        {
            ...
            for (const auto & msg : messages_js)
            {
                ...
                int len =
                    llama_chat_apply_template(chat_tmpl, &chat_message, 1, true, chat_message_output.data() + chat_message_size,
