...
std::vector<std::string> sentences = {"That is a happy dog",
...
int main(int argc, char* argv[]) {
  auto print_usage = [](int argc, char* argv[]) {
    ...
    printf(
        "	--embd-normalize         .i.e --embd-normalize 2 . More "
        "detail, see description above\n");
    printf(
        "	--pooling <type>         .i.e --pooling = mean. More detail, "
  ...
  params.n_batch =
      (params.n_batch < params.n_ctx) ? params.n_ctx : params.n_batch;
  params.n_ubatch = params.n_batch;
  params.pooling_type = params.pooling_type == LLAMA_POOLING_TYPE_NONE
  ...
  auto context_tokenize =
      [&ctx](const std::string& text) -> std::vector<llama_token> {
    const llama_model* model = llama_get_model(ctx);
    const llama_vocab* vocab = llama_model_get_vocab(model);
    if (nullptr == vocab) return {};

    int n_token =
        -llama_tokenize(vocab, text.data(), text.size(), NULL, 0, true, true);
    std::vector<llama_token> tokens(n_token);
    if (llama_tokenize(vocab, text.data(), text.size(), tokens.data(),
  ...
  auto context_batch_to_embedding =
      [&ctx, &params, &embd_print](llama_batch& batch) -> std::vector<float> {
    const llama_model* model = llama_get_model(ctx);
  ...
  while (true) {
    ...
    for (int i = 0; i < sentence_chunks.size(); i++) {
      ...
      float similarity = common_embd_similarity_cos(query_embedding.data(),
    ...
    for (int i = 0; i < std::min(3, (int)similarities.size()); i++) {
      printf("  %.4f: '%s' \n", similarities[i].second,
             sentence_chunks[similarities[i].first].sentence.c_str());
    }
  };
